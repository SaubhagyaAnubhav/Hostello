  ...
  <div className="mt-6 space-y-4" aria-hidden="true">
    {[1, 2, 3].map((item) => (
      <div key={item} className="animate-pulse rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="h-6 w-1/3 rounded bg-slate-200 mb-4"></div>
...
const Notices = () => {
  ...
  const [notices, setNotices] = useState(() => {
    const cachedData = sessionStorage.getItem('hostel_notices_cache');
    return cachedData ? JSON.parse(cachedData) : null;
  });

  
  const [loading, setLoading] = useState(!notices);
  const [isBackgroundUpdating, setIsBackgroundUpdating] = useState(false);
  const [error, setError] = useState('');

  const fetchNotices = async (isManualRetry = false) => {
    try {
      if (!notices || isManualRetry) setLoading(true);
      else setIsBackgroundUpdating(true); 

      const data = await getStudentNotices();
      
      setNotices(data);
      sessionStorage.setItem('hostel_notices_cache', JSON.stringify(data)); 
      setError('');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8 relative overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="mt-6 flex flex-col items-start gap-4 rounded-3xl border border-red-200 bg-red-50 p-6 text-red-700" role="alert">
              ...
              <button 
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-red-100 px-4 py-2 text-sm font-medium text-red-800 hover:bg-red-200 transition-colors"
              >
